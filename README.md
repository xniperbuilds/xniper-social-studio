...
A **32-direction variety engine** (24 palettes · 19 font pairings · motif cookbook) plus eight starter templates — every post a different look. Samples below, each from one brief, rendered straight to PNG:
...
- **A variety engine, not a template.** 32 distinct aesthetic *directions* × 24 palettes × 19 font pairings × layouts × motifs — `ideate.py` samples fresh combinations so two posts never look alike (editorial-vintage, neo-brutalism, riso-print, blueprint-diagram, claymorphism, 3D-depth, liquid-chrome, vaporwave, and more). `--carousel N` plans a whole set in one command.
